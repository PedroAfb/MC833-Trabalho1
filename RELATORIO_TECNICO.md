...
Observacao de ambiente (Linux x macOS):
No Linux, durante os testes com cliente e servidor em sub-redes diferentes (roteadas pelo container roteador), houve falha de conectividade entre os containers para o fluxo da aplicacao. Para garantir estabilidade e concluir os testes funcionais (catalog, stream e metrics), cliente e servidor foram colocados na mesma rede IP do Docker. No macOS, esse ajuste nao foi necessario no mesmo grau.
...
## 5. Uso de IA (declaracao obrigatoria)

Ferramenta utilizada:
- GitHub Copilot (modelo GPT-5.3-Codex)
